...
const ProfileView: React.FC<ProfileViewProps> = ({ onEditProfile, showBackButton, onBack }) => {
  ...
  const { } = useFollow(); // Remove profileRefreshTrigger
  const navigate = useNavigate();
  ...
  // Removed: useEffect untuk check threadId dari URL params
  ...
  const openThreadModal = (thread: Post) => {
    if (activeTab === 'media') {
      // Untuk media tab, hanya set state tanpa URL change
      setSelectedThread(thread);
    } else {
      // Untuk all-post tab, navigate ke full thread page
  ...
  const closeThreadModal = () => {
    ...
    // Dalam media tab, tidak ada URL change
  };
